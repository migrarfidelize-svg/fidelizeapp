-- 1. DROP old variants explicitly
DROP FUNCTION IF EXISTS public.dashboard_summary(uuid, timestamptz);
DROP FUNCTION IF EXISTS public.dashboard_summary(uuid);

-- 2. CREATE the hardened version
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'rewardsCount',  (SELECT count(*) FROM rewards r WHERE r.establishment_id = _est AND r.created_at >= _cutoff),
...
-- 3. REVOKE ALL and GRANT only to service_role
