...
describe("CRM Agent tenant-safe", () => {
  ...
  it("17. Agent usa contexto e configuração do tenant", async () => { await processAgentMessage({ conversationId: "conv-a", inboundText: "ajuda", flowId: "flow-a", stepId: "step-a" }); expect(vi.mocked(generateAgentResponse).mock.calls.at(-1)?.[0].systemPrompt).toContain("Conta A"); });
  it("18. Agent envia e persiste a resposta", async () => { await processAgentMessage({ conversationId: "conv-a", inboundText: "ajuda", flowId: "flow-a", stepId: "step-a" }); expect(send).toHaveBeenCalledOnce(); expect(rows.crm_messages.at(-1).body).toBe("Resposta IA"); });
  it("19. falha do provider não persiste falso sucesso", async () => { send.mockResolvedValue({ ok: false, message: "offline" }); await expect(processAgentMessage({ conversationId: "conv-a", inboundText: "ajuda", flowId: "flow-a", stepId: "step-a" })).rejects.toThrow("offline"); expect(rows.crm_messages).toHaveLength(1); });
