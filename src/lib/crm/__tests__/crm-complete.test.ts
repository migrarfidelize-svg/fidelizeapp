...
describe("CRM Agent tenant-safe", () => {
  ...
  it("19. falha do provider não persiste falso sucesso", async () => { send.mockResolvedValue({ ok: false, message: "offline" }); await expect(processAgentMessage({ conversationId: "conv-a", inboundText: "ajuda", flowId: "flow-a", stepId: "step-a" })).resolves.toEqual({ action: "failed", reason: "offline" }); expect(rows.crm_messages).toHaveLength(1); });
