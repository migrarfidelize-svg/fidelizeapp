...
export async function processAgentMessage(input: { conversationId: string; inboundText: string; flowId: string; stepId: string }) {
  const convResult = await (supabaseAdmin as any).from("crm_conversations").select("*").eq("id", input.conversationId).single();
  ...
  if (handoffKeywords.some((kw: string) => inputLower.includes(kw.toLowerCase()))) {
    return { action: "handoff_requested" };
  }

  const stepResult = await (supabaseAdmin as any).from("crm_flow_steps").select("payload").eq("id", input.stepId)
    .eq("flow_id", input.flowId).eq("establishment_id", conv.establishment_id).maybeSingle();
  if (stepResult.error) throw stepResult.error;
  
  const historyResult = await (supabaseAdmin as any).from("crm_messages").select("body, direction")
