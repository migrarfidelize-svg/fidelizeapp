...
const SUPPORT_TERMS = ["5", "suporte", "atendente", "falar com atendente", "falar com suporte", "humano", "ajuda humana"];
...
async function handoff(conv: any, confirmation: string) {
  ...
  if (!(conv.metadata as any)?.support?.active) await sendAndPersist(conv, confirmation, {}, "support_handoff");
...
export async function processStep(conv: any, step: any, steps: any[]): Promise<FlowActionResult> {
  ...
  switch (stepType(step)) {
    case "message": {
      ...
      const next = steps.find((candidate) => candidate.sort_order === step.sort_order + 1);
...
export async function executeFlow(conversationId: string, messageBody: string): Promise<FlowActionResult> {
  ...
  if (conv.status !== "bot") return { ok: true, action: "ignored" };
  const input = messageBody.trim().toLocaleLowerCase("pt-BR");
  if (SUPPORT_TERMS.some((term) => input === term || (term !== "5" && input.includes(term)))) {
  ...
  if (state?.mode === "agent") {
    ...
    await processAgentMessage({ conversationId, inboundText: messageBody, flowId, stepId: state.stepId });
    return { ok: true, action: "agent" };
  }
  if (state?.stepId) {
