...
import { CourierInstallBar, CourierInstallGate } from "@/components/courier/CourierInstallGate";
...
function CourierLayout() {
  ...
  // Início vira mapa imersivo (tela cheia) quando o entregador está aprovado.
  const immersive = pathname === "/entregador" && courier?.status === "approved";

  return (
    <div className={"bg-background " + (immersive ? "h-[100dvh] overflow-hidden" : "min-h-[100dvh] pb-28")}>
      {!immersive && (
      ...
      <main className={immersive ? "h-full w-full" : "mx-auto w-full max-w-3xl px-4 py-4"}>
        <Outlet />
      </main>

      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/95 backdrop-blur-xl">
        <CourierInstallBar onOpen={() => setGateOpen(true)} />
      ...
      {courier && <CourierInstallGate open={gateOpen} onClose={() => setGateOpen(false)} />}
    </div>
  );
}
