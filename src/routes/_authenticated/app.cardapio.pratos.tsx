...
function PratosPage() {
  ...
  const cats = useQuery({
    ...
    queryFn: () => fetchCats({ data: { establishment_id: estId! } }),
    enabled: !!estId,
  });

  const list = useQuery({
    queryKey: ["menu-items", estId, catFilter],
    queryFn: () => fetchList({
      data: {
  ...
  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["menu-items", estId] });
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => { toast.success(editing ? "Prato atualizado" : "Prato criado"); setOpen(false); setEditing(null); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  const del = useMutation({
    mutationFn: (id: string) => mutDelete({ data: { establishment_id: estId!, id } }),
    onSuccess: () => { toast.success("Prato removido"); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  const toggle = useMutation({
    mutationFn: (p: { id: string; active: boolean }) => mutToggle({ data: { establishment_id: estId!, ...p } }),
    onSuccess: () => invalidate(),
  });
  const dup = useMutation({
    mutationFn: (id: string) => mutDup({ data: { establishment_id: estId!, id } }),
    onSuccess: () => { toast.success("Prato duplicado"); invalidate(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-6">
      <PageHero
        ...
        title="Pratos"
        subtitle="Cadastre cada item com foto, vídeo vertical estilo Stories, preço, ingredientes, badges dietéticos e tempo de preparo."
      />
      ...
      {list.isLoading ? (
        ...
        <EmptyItems onCreate={openNew} hasCats={categories.length > 0} />
      ) : (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((it, idx) => (
              ...
              <CardContent className="space-y-3 p-4">
                ...
                <div className="flex items-center gap-1 pt-1">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Remover prato?</AlertDialogTitle>
...
function EmptyItems({ onCreate, hasCats }: { onCreate: () => void; hasCats: boolean }) {
  return (
    <Card>
      <CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
        <div className="card-icon"><LayoutList className="h-6 w-6" /></div>
        <div>
          <h3 className="text-lg font-semibold">Nenhum prato ainda</h3>
          <p className="mt-1 max-w-md text-sm text-muted-foreground">
            {hasCats
              ? "Cadastre seu primeiro item — foto ou vídeo vertical, descrição curta, preço e badges dietéticos."
              : "Dica: crie primeiro as categorias para organizar melhor. Você também pode cadastrar pratos sem categoria."}
          </p>
        </div>
        <Button onClick={onCreate}><Plus className="mr-2 h-4 w-4" /> Novo prato</Button>
      </CardContent>
    </Card>
  );
}
...
function ItemDialog({
  open, onOpenChange, estId, categories, initial, onSubmit, loading,
}: {
  open: boolean;
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="space-y-2 md:col-span-2">
            <Label>Nome *</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex.: Burger Trufado" maxLength={120} />
          </div>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea value={shortDesc} onChange={(e) => setShortDesc(e.target.value)} rows={2} maxLength={200}
              placeholder="Uma linha que abre o apetite (aparece no card do cardápio)." />
          </div>
