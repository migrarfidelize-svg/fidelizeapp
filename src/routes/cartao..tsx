...
function getEstablishmentErrorCode(error: unknown) {
  ...
  if (raw.includes("CAMPAIGN_NOT_FOUND")) {
...
export const Route = createFileRoute("/cartao/")({
  ...
  loader: async ({ params, context }) => {
    ...
    try {
      ...
      const code =
        getEstablishmentErrorCode(error);
...
function PublicPromoCard({
  ...
  const media = (promo.media || []).filter((m: any) => !!m.url);
  const current = media[idx];
  const combined = [
    ...(promo.external_links || []),
    ...globalLinks.filter((g) => !(promo.external_links || []).some((p: any) => p.url === g.url)),
  ];
  ...
  return (
    <li className="overflow-hidden rounded-2xl border bg-background">
      ...
      <div className="p-4">
        <h3 className="font-display font-bold">{promo.title}</h3>
        {promo.body && <p className="mt-1 text-sm text-muted-foreground line-clamp-2">{promo.body}</p>}
        {combined.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {combined.map((l, i) => (
              <Button key={`${l.url}-${i}`} asChild size="sm" variant="outline" className="h-8 rounded-full text-xs">
                <a href={l.url} target="_blank" rel="noopener noreferrer">
                  <ExternalLink className="mr-1.5 h-3 w-3" /> {l.label}
                </a>
              </Button>
            ))}
          </div>
