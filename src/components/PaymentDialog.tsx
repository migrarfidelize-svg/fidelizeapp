  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      try {
        ...
        setStatus(r.status);
        if (r.status === "approved") { toast.success("Pagamento aprovado! Plano ativado."); qc.invalidateQueries({ queryKey: ["billing"] }); qc.invalidateQueries({ queryKey: ["dashboard"] }); qc.invalidateQueries({ queryKey: ["establishment"] }); setTimeout(onDone, 1200); }
