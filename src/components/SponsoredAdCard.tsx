...
export function SponsoredAdCard({ data, model, className }: SponsoredAdCardProps) {
  ...
  const SponsoredBadge = () => (
    <div className={cn(
      "inline-flex items-center px-2 py-0.5 rounded-full backdrop-blur-md border border-white/10 bg-black/30 text-white/90 z-20"
    )}>
  ...
  const CTAButton = ({ size = "md" }: { size?: "sm" | "md" }) => {
    ...
    return (
      <button className={cn(
        "bg-primary text-primary-foreground font-black uppercase tracking-widest shadow-xl shadow-primary/30 active:scale-95 transition-all flex items-center justify-center gap-2 group whitespace-nowrap border border-white/10 hover:shadow-primary/40",
  ...
  if (model === "premium_banner") {
    return (
      ...
      >
        ...
        {/* Professional Overlay / Gradient Base */}
          ...
          <div className="flex items-end justify-between gap-8">
            <div className="space-y-4 flex-1 max-w-[70%]">
              <div className="space-y-1">
                <h3 className="text-white text-3xl font-display font-black leading-[1.1] tracking-tight drop-shadow-md line-clamp-2">
                  {data.title}
                </h3>
                {data.description && (
                  <p className="text-white/60 text-xs font-medium line-clamp-1 drop-shadow-sm">
                    {data.description}
                  </p>
                )}
        ...
        <div className="absolute inset-0 p-6 flex flex-col justify-between">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              {data.description && (
                <p className="text-white/50 text-[10px] font-medium line-clamp-1">
                  {data.description}
                </p>
              )}
    ...
    >
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-between">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            <h3 className="text-white text-[13px] font-display font-black leading-tight line-clamp-2">
              {data.title}
            </h3>
