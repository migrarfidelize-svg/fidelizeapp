...
/**
 * Gate de plano compartilhado pelo Cardápio Virtual e pelo Catálogo Digital.
 * Ambos usam a mesma base; só mudam os rótulos e o recurso de plano exigido.
 */
export function ShowcaseGate({ kind }: { kind: ShowcaseKind }) {
  ...
  const { allowed, viaPlan, isLoading } = useMyFeature(est?.id, L.feature);
      ...
      <>
        {!viaPlan && (
          <div className="mx-4 mt-4 md:mx-8 rounded-lg border border-primary/30 bg-primary-soft/40 px-4 py-3 text-sm flex items-start gap-2">
            ...
            <span>
              Acesso liberado pela equipe Fidelize para montar seu {L.eyebrow.toLowerCase()}. Para{" "}
    ...
    <div className="mx-auto max-w-2xl p-4 md:p-8">
      <Card className="border-primary/30">
        <CardContent className="p-8 text-center space-y-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{L.module}</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Este recurso não está incluído no seu plano atual. Faça upgrade para publicar sua
              vitrine online, com fotos, {L.categories.toLowerCase()}, QR Code próprio e
          ...
          <ul className="text-sm text-left space-y-2 mx-auto max-w-sm">
            <li className="flex gap-2">
              ...
              Página pública {L.publicBase}/{est.slug}
            </li>
            <li className="flex gap-2">
              <Sparkles className="h-4 w-4 text-primary shrink-0 mt-0.5" />
              Modelos prontos por segmento e aparência 100% personalizável
            </li>
          </ul>
