...
export function ShowcaseCategories({ kind }: { kind: ShowcaseKind }) {
  ...
  const list = useQuery({
    ...
    queryFn: () => fetchList({ data: { establishment_id: estId!, kind } }),
    enabled: !!estId,
  });
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => {
      toast.success(`${L.category} ${editing ? "atualizada" : "criada"}`);
      setOpen(false); setEditing(null);
      qc.invalidateQueries({ queryKey: ["menu-categories", estId, kind] });
  ...
  const del = useMutation({
    ...
    onSuccess: () => {
      toast.success(`${L.category} removida`);
      qc.invalidateQueries({ queryKey: ["menu-categories", estId, kind] });
      qc.invalidateQueries({ queryKey: ["menu-overview", estId, kind] });
    },
    onError: (e: Error) => toast.error(e.message),
  ...
  const move = useMutation({
    mutationFn: (p: { id: string; direction: "up" | "down" }) => mutMove({ data: { establishment_id: estId!, kind, ...p } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["menu-categories", estId, kind] }),
  });
  ...
  return (
    <div className="space-y-6">
      <PageHero
        ...
        eyebrow={L.module}
        title={L.categories}
        subtitle={kind === "catalog"
          ? "Organize sua loja em coleções (Lançamentos, Acessórios, Promoções...). Cada uma pode ter imagem de capa e ser marcada como destaque."
          : "Organize seu cardápio em seções (Entradas, Pratos principais, Bebidas...). Cada uma pode ter imagem de capa e ser marcada como destaque."}
      />

      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          {cats.length === 0
            ? `Nenhuma ${L.category.toLowerCase()} ainda.`
            : `${cats.length} ${(cats.length === 1 ? L.category : L.categories).toLowerCase()}`}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <MenuTemplatePicker establishmentId={estId} kind={kind} />
          <Button onClick={openNew} disabled={!estId}>
            <Plus className="mr-2 h-4 w-4" /> Nova {L.category.toLowerCase()}
          </Button>
        </div>
      </div>
      ...
      {list.isLoading ? (
        ...
        <EmptyState onCreate={openNew} kind={kind} />
      ) : (
                ...
                <CardContent className="flex min-w-0 flex-col justify-between gap-2 p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Remover {L.category.toLowerCase()}?</AlertDialogTitle>
                          <AlertDialogDescription>
                            Os {L.itemsLower} desta {L.category.toLowerCase()} ficarão sem {L.category.toLowerCase()}, mas não serão apagados.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
...
function EmptyState({ onCreate, kind }: { onCreate: () => void; kind: ShowcaseKind }) {
  const L = showcase(kind);
  return (
    <Card>
      <CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <div>
          <h3 className="text-lg font-semibold">Crie sua primeira {L.category.toLowerCase()}</h3>
          <p className="mt-1 max-w-md text-sm text-muted-foreground">
            {kind === "catalog"
              ? "Comece por grupos amplos como Lançamentos, Mais vendidos, Acessórios e Promoções."
              : "Comece por seções amplas como Entradas, Pratos principais, Bebidas e Sobremesas."}{" "}
...
function CategoryDialog({
  open, onOpenChange, estId, initial, onSubmit, loading, kind,
}: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{initial ? `Editar ${L.category.toLowerCase()}` : `Nova ${L.category.toLowerCase()}`}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>Nome *</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={kind === "catalog" ? "Ex.: Acessórios" : "Ex.: Pratos principais"} maxLength={80} />
          </div>
          <div className="space-y-2">
            <Label>Descrição curta</Label>
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder={kind === "catalog" ? "Ex.: Os acessórios mais procurados da loja." : "Ex.: Nossas receitas de assinatura, servidas quentes."} maxLength={400} rows={2} />
          </div>
          <div className="space-y-2">
