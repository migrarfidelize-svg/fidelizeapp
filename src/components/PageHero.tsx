  ...
  return (
    <section className="dash-hero p-6 sm:p-8">
      ...
      {ticker && ticker.length > 0 && (
        <div
          className="hero-reveal relative mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm"
          style={{ animationDelay: "120ms" }}
        >
          {ticker.map((t) => (
            <div
              key={t.label}
              className="flex items-center gap-3 rounded-lg border border-primary/15 bg-[color:color-mix(in_oklab,var(--card)_60%,transparent)] px-3 py-2.5 backdrop-blur-sm"
            >
