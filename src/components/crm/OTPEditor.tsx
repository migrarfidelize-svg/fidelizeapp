...
export function OTPEditor() {
  ...
  const sendTestMutation = useMutation({
    mutationFn: () => sendOTPTestMessage({ data: { phone: testPhone, message: template.replace(/{{code}}/g, "123456").replace(/{{minutes}}/g, "10").replace(/{{brand}}/g, "Afidelize") } }),
      ...
      <Card className="dash-card">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Code className="h-4 w-4" /> Editor de Template
          </CardTitle>
          <Button 
            variant="ghost" 
            size="sm" 
            ...
            onClick={() => {
              if (confirm("Restaurar o template para o padrão de fábrica?")) {
                setTemplate("Afidelize\n\nSeu código de acesso é {{code}}.\n\nEle expira em {{minutes}} minutos.\n\nNão compartilhe este código.");
              }
            }}
          >
            Restaurar Padrão
        ...
        <Card className="dash-card bg-primary/5 border-primary/20">
          ...
          <CardContent>
            <div className="bg-card border rounded-2xl p-4 shadow-sm relative">
              ...
              <p className="text-xs whitespace-pre-wrap">{template.replace(/{{code}}/g, "123456").replace(/{{minutes}}/g, "10").replace(/{{brand}}/g, "Afidelize")}</p>
            </div>
          </CardContent>
        </Card>
